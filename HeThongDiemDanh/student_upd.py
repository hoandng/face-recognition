...
class Student:
    def __init__(self,root):
        ...
        img3 = PIL.Image.open(path_root + r"ImageFaceDetect\bgnt.png")
        ...
        # ==================================heading====================================
        # ====time====
        img_time = PIL.Image.open(path_root + r"ImageFaceDetect\timsearch50.png")
    ...
    def generate_dataset(self):
        if self.var_dep.get()=="Chọn chuyên ngành" or self.var_std_name.get()=="" or self.var_std_id.get()=="":
            ...
            try:
                ...
                conn.close()
                #=========load haar===================
                face_classifier=cv2.CascadeClassifier(path_root + "haarcascade_frontalface_default.xml")
                ...
                while True:
                    ...
                    if face_cropped(my_frame) is not None:
                        ...
                        folder_path = os.path.join(path_root, "data", "student_" + f"{id}")  # data/name-id

                        # Tạo thư mục nếu chưa tồn tại
                        os.makedirs(folder_path, exist_ok=True)  

                        # Tạo đường dẫn file ảnh
                        file_name_path = os.path.join(folder_path, f"{img_id}.jpg")
                        print(file_name_path)
                        cv2.imwrite(file_name_path,face)
                        cv2.putText(face,str(img_id),(50,50),cv2.FONT_HERSHEY_SIMPLEX,2,(0,255,0),2)
                        cv2.imshow("Cropped Face",face)

                    if cv2.waitKey(1)==13 or int(img_id)==50:#duyet du 120 anh
                        break
                cap.release()
    ...
    def train_classifier(self):
        ...
        data_dir = os.path.join(path_root, "data", "student_" + f"{id}")
        
        # Kiểm tra thư mục có tồn tại không
        if not os.path.exists(data_dir):
            messagebox.showerror("Lỗi", f"Không tìm thấy dữ liệu cho student_{id}", parent=self.root)
            return
        ...
        for image in path:
            img = PIL.Image.open(image).convert('L')  # Chuyển ảnh về grayscale
            imageNp = np.array(img, 'uint8')  # Chuyển thành mảng numpy
        ...
        # =============== Nạp mô hình cũ nếu có =================
        clf = cv2.face.LBPHFaceRecognizer_create()
        model_path = os.path.join(path_root, "classifier.xml")

        if os.path.exists(model_path):
            clf.read(model_path)  # Nạp mô hình cũ trước khi train thêm

        # ================ Train với dữ liệu mới =================
        clf.update(faces, ids)  # Cập nhật dữ liệu mới mà không xóa dữ liệu cũ
        clf.write(model_path)  # Lưu lại model

        cv2.destroyAllWindows()
        messagebox.showinfo("Kết quả", f"Training student_{id} completed!", parent=self.root)
