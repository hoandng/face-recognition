...
class Attendance:
    ...
    def exportCsv(self):
        # try:
            ...
            with open(path_root + 'Attendance_CSV/diemdanh.csv',mode="w",newline="",encoding="utf-8") as myfile:
    ...
    def openImage(self):
        if self.var_atten_id == "":
            ...
            if not os.path.exists(path_root + "DiemDanhImage\ " + self.var_atten_id.get() +"Ra" +".jpg"):
                messagebox.showerror("Lỗi", "Không tìm thấy ảnh điểm danh", parent=self.root)
            else:
                img = cv2.imread(path_root + "DiemDanhImage\ " + str(self.var_atten_id.get()) +"Ra"+ ".jpg")
                img = cv2.resize(img, (300, 300))
                cv2.imshow("Out of Class", img)
        elif(self.var_atten_timeout.get()=='None'):
            if not os.path.exists(path_root + "DiemDanhImage\ " + self.var_atten_id.get() + ".jpg"):
                messagebox.showerror("Lỗi", "Không tìm thấy ảnh điểm danh", parent=self.root)
            else:
                img1=cv2.imread(path_root + "DiemDanhImage\ " + str(self.var_atten_id.get()) + ".jpg")
